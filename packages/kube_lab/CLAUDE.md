...
2. その中に入っている `gpu-dev` CLI（Python パッケージ `ssh-tool`）— GPU Pod を起こして exec で入るツール
...
- **ServiceAccount トークンから in-cluster kubeconfig を生成する**（`~/.kube/config`、
  `tokenFile` 参照、current-context の namespace は `K8S_NAMESPACE`）。`gpu-dev` の kubectl 権限は
...
`gpu_dev_k8s.run()` が実行コマンドを `[cmd] ...` として表示する。この方式は kube-sshuser と同じ流儀。

モジュール構成（`gpu_dev_core.py` は再エクスポートするだけのハブで、ロジックを持たない）:

- `gpu_dev.py` — argparse、`--file` の YAML マージ、`up` の本体フロー
- `gpu_dev_pod.py` — Pod マニフェスト生成と `ensure_pod()`
- `gpu_dev_k8s.py` — kubectl ラッパ（apply / exists / phase / delete / port-forward）
- `gpu_dev_identity.py` — owner・Pod 名・namespace の決定
- `gpu_dev_listing.py` — `status` の表示整形
- `gpu_dev_defaults.py` — 既定値の**唯一の定義場所**

**識別の規約**（変えるなら全部まとめて）:

- owner = `$USER`（無ければ `$LOGNAME`）を `sanitize_k8s_name()` にかけたもの。**k8s の認証主体ではなく
  シェルの環境変数**なので、sudo 経由だと owner が変わり、自分の Pod が見えなくなる
- Pod 名 = `gpu-dev-<owner>` / `--name` 指定時は `gpu-dev-<owner>-<name>`
...
明示的に渡すと YAML 側に上書きされる。既定値を `gpu_dev_defaults.py` で変えると、この判定
