...
これは [kube-tools](https://github.com/hiroshima-aidi/kube-tools) モノレポの 1 パッケージです。

## モノレポ内での担当範囲

| 場所 | 役割 | 使う人 |
|---|---|---|
| **`packages/kube_sshuser`**（本パッケージ） | 学生ごとの namespace / PVC / クォータ / SSH 環境の払い出しと管理 | 管理者 |
| `packages/kube_lab` | `gpu-dev`（GPU Pod の起動・停止）。SSH イメージに焼き込まれる | 利用者（学生） |
| `images/ssh` | SSH コンテナイメージ | 管理者 |
| `packages/kube_jupyterhub` | JupyterHub の管理 | 管理者（別系統） |
| `images/jupyter` | Jupyter コンテナイメージのビルド | 管理者（別系統） |

**JupyterLab / JupyterHub のデプロイは本パッケージの対象外**で、`kube_jupyterhub` が担当します。

本ツールが作るのは「入れ物」（namespace・PVC・クォータ・SSH の入口）までです。
その中で GPU Pod を起動するのは利用者側の `gpu-dev` で、こちらは `docs/user/kube-lab.md` を参照してください。
