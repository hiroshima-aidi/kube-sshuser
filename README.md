...
このリポジトリには admin-tool 本体のみを含みます。

## できること

- `admin-tool create`: namespace / PVC / ResourceQuota / SA / RBAC / SSH Deployment を作成
- `admin-tool delete`: 作成済み環境の削除
- `admin-tool show`: ユーザ単位のレジストリ情報表示
- `admin-tool list`: レジストリ一覧表示（status フィルタ対応）
...
```bash
pip install "git+https://github.com/hiroshima-aidi/admin-tool.git"
```
...
/opt/venv/bin/pip install "git+https://github.com/hiroshima-aidi/admin-tool.git"
```
...
```bash
admin-tool create taro --public-key-file /path/to/key.pub --image ghcr.io/hiroshima-aidi/ssh-for-k8s:latest --port 2222
```
...
```bash
admin-tool show taro
admin-tool show taro --json

admin-tool list
admin-tool list --status active
admin-tool list --json
```

## 主なオプション

`admin-tool create <user> ...` の主なオプション:
...
`admin-tool delete <user> ...` の主なオプション:
...
既定では `--out-dir ./out` 配下に以下を出力します。

- `./out/<user>/provision-<user>.yaml`: 生成マニフェスト
- `./out/_registry/users/<user>.json`: ユーザの最新状態
- `./out/_registry/events.ndjson`: 監査イベントログ
